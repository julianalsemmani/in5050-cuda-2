...
__device__ static void dct_1d(float *in_data, float *out_data, int i, int j)
{
  __shared__ float dct[8];

  // dct[i] += in_data[j] * dctlookup_d[j][i];
  atomicAdd(&dct[i], in_data[j] * dctlookup_d[j][i]);
